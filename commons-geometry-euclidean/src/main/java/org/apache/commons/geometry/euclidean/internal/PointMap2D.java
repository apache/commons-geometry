...
public class PointMap2D<V> extends MultiDimensionalEuclideanPointMap<Vector2D, V> {

    public PointMap2D(final Precision.DoubleEquivalence precision) {
        super(precision);
    }

    /** {@inheritDoc} */
    @Override
    protected PointMapNode<Vector2D, V> createNode() {
        return new PointMapNode2D<>(this);
    }

    private static final class PointMapNode2D<V> extends PointMapNode<Vector2D, V> {
        ...
        PointMapNode2D(final PointMap2D<V> map) {
            super(map);
        }
        ...
        protected Vector2D computeSplitPoint(final List<Entry<Vector2D, V>> entries) {
            ...
            for (Entry<Vector2D, V> entry : entries) {
