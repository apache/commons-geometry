...
public final class RegionBSPTreeSubLine3D extends SubLine3D {
     ...
     *      as this instance.
     */
    public List<ConvexSubLine3D> toConvex() {
        final List<Interval> intervals = region.toIntervals();

        final Line3D line = getLine();
        final List<ConvexSubLine3D> convex = new ArrayList<>(intervals.size());

        for (final Interval interval : intervals) {
            convex.add(ConvexSubLine3D.fromInterval(line, interval));
        }
