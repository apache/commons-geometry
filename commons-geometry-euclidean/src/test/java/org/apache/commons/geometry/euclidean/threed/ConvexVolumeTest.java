...
public class ConvexVolumeTest {
    ...
    private static void checkClassify(ConvexVolume vol, RegionLocation loc, Vector3D ... pts) {
        for (Vector3D pt : pts) {
            Assert.assertEquals("Unexpected location for point " + pt, loc, vol.classify(pt));
        }
    }
}
