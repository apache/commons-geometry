...
public class SphericalPolygonsSetTest {
    ...
    public void testFullSphere() {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            Assert.assertEquals(Location.INSIDE, full.checkPoint(Point2S.from(v)));
        }
    ...
    public void testEmpty() {
        ...
        for (int i = 0; i < 1000; ++i) {
            Vector3D v = Vector3D.of(random.nextVector());
            Assert.assertEquals(Location.OUTSIDE, empty.checkPoint(Point2S.from(v)));
        }
    ...
    public void testSouthHemisphere() {
        ...
        for (int i = 0; i < 1000; ++i) {
            Vector3D v = Vector3D.of(random.nextVector());
            if (v.getZ() < -sinTol) {
                Assert.assertEquals(Location.INSIDE, south.checkPoint(Point2S.from(v)));
            } else if (v.getZ() > sinTol) {
                Assert.assertEquals(Location.OUTSIDE, south.checkPoint(Point2S.from(v)));
            } else {
                Assert.assertEquals(Location.BOUNDARY, south.checkPoint(Point2S.from(v)));
            }
        }
    ...
    public void testPositiveOctantByIntersection() {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            if ((v.getX() > sinTol) && (v.getY() > sinTol) && (v.getZ() > sinTol)) {
                Assert.assertEquals(Location.INSIDE, octant.checkPoint(Point2S.from(v)));
            } else if ((v.getX() < -sinTol) || (v.getY() < -sinTol) || (v.getZ() < -sinTol)) {
                Assert.assertEquals(Location.OUTSIDE, octant.checkPoint(Point2S.from(v)));
            } else {
                Assert.assertEquals(Location.BOUNDARY, octant.checkPoint(Point2S.from(v)));
            }
        }
        ...
        Assert.assertEquals(0.0,
                            octant.getBarycenter().distance(Point2S.from(Vector3D.of(1, 1, 1))),
                            TEST_EPS);
        ...
        Assert.assertEquals(0, reversedCap.getCenter().distance(Point2S.from(Vector3D.of(-1, -1, -1))), TEST_EPS);
    ...
    public void testNonConvex() {
        ...
        for (int i = 0; i < 1000; ++i) {
            Vector3D v = Vector3D.of(random.nextVector());
            if (((v.getX() < -sinTol) || (v.getY() < -sinTol)) && (v.getZ() > sinTol)) {
                Assert.assertEquals(Location.INSIDE, threeOctants.checkPoint(Point2S.from(v)));
            } else if (((v.getX() > sinTol) && (v.getY() > sinTol)) || (v.getZ() < -sinTol)) {
                Assert.assertEquals(Location.OUTSIDE, threeOctants.checkPoint(Point2S.from(v)));
            } else {
                Assert.assertEquals(Location.BOUNDARY, threeOctants.checkPoint(Point2S.from(v)));
            }
        }
    ...
    public void testModeratlyComplexShape() {
        ...
        Assert.assertEquals(Location.OUTSIDE, polygon.checkPoint(Point2S.from(Vector3D.of( 1,  1,  1).normalize())));
        Assert.assertEquals(Location.INSIDE,  polygon.checkPoint(Point2S.from(Vector3D.of(-1,  1,  1).normalize())));
        Assert.assertEquals(Location.INSIDE,  polygon.checkPoint(Point2S.from(Vector3D.of(-1, -1,  1).normalize())));
        Assert.assertEquals(Location.INSIDE,  polygon.checkPoint(Point2S.from(Vector3D.of( 1, -1,  1).normalize())));
        Assert.assertEquals(Location.OUTSIDE, polygon.checkPoint(Point2S.from(Vector3D.of( 1,  1, -1).normalize())));
        Assert.assertEquals(Location.OUTSIDE, polygon.checkPoint(Point2S.from(Vector3D.of(-1,  1, -1).normalize())));
    ...
    public void testSeveralParts() {
        ...
        for (int i = 0; i < 1000; ++i) {
            Vector3D v = Vector3D.of(random.nextVector());
            if ((v.getX() < -sinTol) && (v.getY() < -sinTol) && (v.getZ() < -sinTol)) {
                Assert.assertEquals(Location.INSIDE, polygon.checkPoint(Point2S.from(v)));
            } else if ((v.getX() < sinTol) && (v.getY() < sinTol) && (v.getZ() < sinTol)) {
                Assert.assertEquals(Location.BOUNDARY, polygon.checkPoint(Point2S.from(v)));
            } else if ((v.getX() > sinTol) && (v.getY() > sinTol) && (v.getZ() > sinTol)) {
                Assert.assertEquals(Location.INSIDE, polygon.checkPoint(Point2S.from(v)));
            } else if ((v.getX() > -sinTol) && (v.getY() > -sinTol) && (v.getZ() > -sinTol)) {
                Assert.assertEquals(Location.BOUNDARY, polygon.checkPoint(Point2S.from(v)));
            } else {
                Assert.assertEquals(Location.OUTSIDE, polygon.checkPoint(Point2S.from(v)));
            }
        }
    ...
    public void testPartWithHole() {
        ...
        Point2S center = Point2S.from(Vector3D.of(1, 1, 1));
    ...
    public void testConcentricSubParts() {
        ...
        double phi = Point2S.from(center).getPolar();
